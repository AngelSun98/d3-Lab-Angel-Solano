...
class Army:
    ...
    def battleArmy(self, enemy) -> None:
        enemyArmy = enemy.army
        enemyCenter = enemy.getCenter()
        for x in range(len(self._army)):
            battle = False
            for y in range(len(enemyArmy)):
                if (enemyArmy[y].health > 0):
                    vector = [self._army[x].x - enemyArmy[y].x, self._army[x].y - enemyArmy[y].y]
                    distance = math.sqrt(vector[0] ** 2 + vector[1] ** 2)
                    if (distance <= self._army[x]._attack_range):
                        enemyArmy[y].GetDamange(self._army[x].battle(enemyArmy[y].protection))
                        battle = True
            if battle == False:
                self._army[x].move(enemyCenter)
